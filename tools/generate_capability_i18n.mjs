...
const labels = {
  ...
  en: {
    ...
    ctaTitle: "Start with the execution gap.",
    ctaBody: "Send the brand, product category, target area, and first issue to solve. We will map the operating sequence required for Vietnam.",
  ...
  <header class="fixed left-0 top-0 z-50 w-full border-b border-line/80 bg-white/95 backdrop-blur">
    <nav class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8" aria-label="Primary navigation">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="mobile-menu-button lg:hidden" type="button" data-mobile-toggle aria-controls="mobile-menu" aria-expanded="false" aria-label="${lang === "ko" ? "메뉴 열기" : "Open navigation"}">
    ...
    <div id="mobile-menu" class="mobile-nav-panel lg:hidden" data-mobile-menu hidden>
      <a href="${homeUrl}#leadership"><span>${esc(l.about)}</span><span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span></a>
      <a href="${homeUrl}#local-partner"><span>${esc(l.partnership)}</span><span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span></a>
      <a href="${homeUrl}#capabilities"><span>${esc(l.business)}</span><span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span></a>
      <a href="${homeUrl}#proof"><span>${esc(l.record)}</span><span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span></a>
      <a href="../../contact.html"><span>${esc(l.contact)}</span><span class="material-symbols-outlined text-lg" aria-hidden="true">north_east</span></a>
    </div>
  </header>
